<x-modal id="contact" class="bg-foreground ">
    <div
        class="mb-[25px] pt-[50px] lg:mb-[40px] col-span-2 text-base flex flex-wrap justify-between leading-6 gap-8"
        id="modal-title">
        @if(Session::has('status'))
            <p x-data="{init: function(){
                return $dispatch('modal-contact');
            }}" class="text-primary alert alert-info">{!! Session::get('status') !!}</p>
        @else
